...
st.write(
    "Compute cold outlet temperature, hot outlet temperature, and optional cold-side mass flow "
    "using the effectiveness-NTU method for a counter-flow heat exchanger."
)
...
if submitted:
    try:
        if thi <= tci:
            ...
            st.error("Both U and Area must be greater than zero.")
        else:
            if mode.startswith("Known cold-flow rate"):
            ...
            d1.metric("UA (W/K)", f"{r['UA']:.2f}")
            d2.metric("C_h (W/K)", f"{r['C_h']:.2f}")
            d3.metric("C_c (W/K)", f"{r['C_c']:.2f}")
            d4.metric("NTU", f"{r['NTU']:.4f}")

            e1, e2 = st.columns(2)
            e1.metric("Capacity ratio, C_r", f"{r['C_r']:.4f}")
            e2.metric("Effectiveness ε", f"{r['epsilon']:.4f}")

            st.caption(
                "Assumptions: steady-state operation, constant specific heats, "
                "no heat loss to surroundings, counter-flow exchanger, and known U and Area."
            )
